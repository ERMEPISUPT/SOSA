...
public class ControllerPdf extends HttpServlet {
    ...
    public static final String FIRMA = "VistaReservas/editar.jsp";
    public static final String PDF = "/pdf.jsp";
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equalsIgnoreCase("firma")) {
            forward = FIRMA;
            int studentIdM = Integer.parseInt(request.getParameter("id"));
            id_pdf = studentIdM;
            PdfVO pdfvo = pdfdao.getPdfVOById(studentIdM);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        sql auto = new sql();
        int nuevoid = auto.auto_increm("SELECT MAX(codigopdf) FROM empleado;");
        
        try{
            String name = request.getParameter("txtdni");
            pdfvo.setNombrepdf(name);
            String direccion = request.getParameter("txtestado");
            pdfvo.setDireccionpdf(direccion);
            int firma = Integer.parseInt(request.getParameter("txtfirma"));
            pdfvo.setFirmaAdmin(firma);
        }catch(Exception ex){
        ...
        try {
            ...
            if (estado.equalsIgnoreCase("firma")) {
                pdfvo.setCodigopdf(id_pdf);
                if (inputStream != null) {
                    pdfdao.Firmar_PdfVO(pdfvo);
                } else {
                    pdfdao.Firmar_PdfVO2(pdfvo);
                }
            }
        } catch (Exception ex) {
            System.out.println("textos: "+ex.getMessage());
